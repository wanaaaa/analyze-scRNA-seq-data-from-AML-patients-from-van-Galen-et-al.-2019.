...
# soList <- list(AML419_so, AML707_so)
Seurat_so <- merge(AML419_so, AML707_so)
...
Seurat_so <- RunUMAP(Seurat_so, dims = 1:30, verbose = FALSE)
# 
DimPlot(Seurat_so, reduction = "umap", group.by = "CellType", shuffle = TRUE) + theme(aspect.ratio = 1) + scale_color_igv()
DimPlot(Seurat_so, reduction = "umap", group.by = "PredictionRefined", shuffle = TRUE) + theme(aspect.ratio = 1) + 
  scale_color_igv()
DimPlot(Seurat_so, reduction = "umap", shuffle = TRUE) + theme(aspect.ratio = 1) + scale_color_igv()
